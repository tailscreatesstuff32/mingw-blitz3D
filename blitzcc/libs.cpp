//DONE FOR NOW NEED TO UNCOMMENT STUFF AFTER FINISHED.
...
static Type* getTypeFromChar(int c) { //static Type *typeof( int c ){
    switch (c) {
...
//		//global!
		...
		if( !isalpha( s[0] ) ){ start=1;t=getTypeFromChar( s[0] ); }
		for( k=1;k<s.size();++k ){
...
const char* linkUserLibs() {

_ulibkws.clear();

	WIN32_FIND_DATA fd;

	HANDLE h=FindFirstFile( R"(../../userlibs/*.decls)",&fd );
...
//	home=string(p);
	...
	linkerHMOD=LoadLibrary( R"(.\Linker_NEW.dll)");
	if( !linkerHMOD ) return "Unable to open Linker_NEW.dll";

	typedef Linker *(_cdecl*GetLinker)();
	GetLinker gl=(GetLinker)GetProcAddress( linkerHMOD,"linkerGetLinker" );
	if( !gl ) return "Error in Linker_NEW.dll";
	linkerLib=gl();

	runtimeHMOD=LoadLibrary( R"(.\runtime.dll)" );
	//runtimeHMOD=LoadLibrary( R"(.\runtime_NEW.dll)" );
