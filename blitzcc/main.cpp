...
#include "libs.h" //FIN PORTED

#include <iostream>

#include "../config/config.h"  //FIN PORTED
#include "../stdutil/stdutil.h"  //FIN PORTED
...
#include "../compiler/assem_x86/assem_x86.h" //FIN PORTED
#include "../compiler/codegen_x86/codegen_x86.h" //FIN PORTED
...
//runtimeLib->execute( NULL,NULL,NULL );
	try{
		...
		environ=prog->semant( runtimeEnviron ); //FIN

        //translate
		if( !veryquiet ) cout<<"Translating..."<<endl;
		qstreambuf qbuf;
		iostream asmcode( &qbuf );
		Codegen_x86 codegen( asmcode,debug ); //FIN

        prog->translate( &codegen,userFuncs ); //FIN
		...
		if( !veryquiet ) cout<<"Assembling..."<<endl;
       	module=linkerLib->createModule(); //FIN
		Assem_x86 assem( asmcode,module ); //FIN
		assem.assemble(); //FIN
...
delete prog;
   ...
   if( out_file.size()){
		...
		//FIN/////////////////////////////////////////////////////////////////////////////////////////////////
		//////////////////////////////////////////////////////////////////////////////////////////////////////
		if( !module->createExe_test( out_file.c_str() , R"(.\runtime.dll)" ) ) { //if( !module->createExe_test( R"(.\test.exe)", R"(.\runtime.dll)" ) ){
		//if( !module->createExe_test( out_file.c_str() , R"(.\runtime_NEW.dll)" ) ) { //if( !module->createExe_test( R"(.\test.exe)", R"(.\runtime.dll)" ) ){
		...
		if( debug ){
			dbgHandle=LoadLibrary(  R"(.\debugger_NEW.dll)" );
			if( dbgHandle ){
